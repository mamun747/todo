...
function MainArea() {
  ...
  const [editinId, setEditingId] = useState<number | null>(null);
  ...
  return (
    <section className="pt-10 px-5">
      <div className="max-w-md relative">
        <div className="absolute -right-32 top-20 text-lg">
          Total Tasks:{" "}
        ...
        <div className="flex gap-5 md:flex-row flex-col">
          ...
          >
            ...
            <span className="absolute -bottom-20 left-1/2 transform -translate-x-1/2 text-xs text-white bg-red-500 px-1 py-1 rounded opacity-0 group-hover:opacity-100 transition-all invisible group-hover:visible w-64">
              Note: if once select it you just can see your completed tasks but
              your all incompleted tasks will be gone. So be careful!
            </span>
          </button>
        </div>
        ...
        <div className="mt-5 rounded-md">
          {filteredTasks.length === 0 ? (
            ...
            filteredTasks.map((value, index) => {
              if (!value?.id) return null; // Guard against invalid id
              return (
                <div
                  key={value.id}
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (value.id !== undefined) {
                            handleSave(value.id); // Only call handleSave if id is valid
                          }
                        }}
                      >
                        Save
